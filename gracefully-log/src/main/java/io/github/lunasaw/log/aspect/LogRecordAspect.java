...
public class LogRecordAspect implements ApplicationContextAware {
    ...
    private Object execute(ProceedingJoinPoint joinPoint) {
        ...
        try {
            ...
            OperatorLog operatorLog = OperatorLog.builder().level(LogLevel.ERROR).bizNo(bizNo).operator(operator).fail(fail).category(category).condition(condition).build();
        ...
        OperatorLog operatorLog = OperatorLog.builder().level(LogLevel.INFO).bizNo(bizNo).operator(operator).success(success).category(category).condition(condition).build();
